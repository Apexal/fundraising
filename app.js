...
requireAdmin = function(req, res, next) {
  // if user is authenticated in the session, carry on
  if (req.isAuthenticated() && req.user.verified && req.user.admin)
    return next();
  req.session.redirect = req.originalUrl;
  req.flash(
    'error',
...
app.get(
  ...
  (req, res) => {
    req.db.User.find({
      'application.superior': req.user._id,
      applying: false
    })
      .exec()
      .then(applicants => {
        req.session.applicantCount = res.locals.applicantCount =
          applicants.length;
...
for (let index in files) {
  ...
  const name = (path == 'index.js' ? '' : path.replace('.js', '')).replace(
    '/index',
    ''
  );
  try {
...
app.use((err, req, res, next) => {
  ...
  if (err.message == 'on_slack_but_didnt_apply') {
    req.flash(
      'error',
      'You are somehow on the Kids Tales Slack but have not yet applied and been accepted on the website yet! Make sure you apply first.'
    );
